...
public class ForumOODao {
    ...
    public Forum getForum(int forumId){
        /* ③执行查询并返回结果 */
        return forumQuery.findObject(forumId);
    }

    /** ④定义MappingSqlQuery */
    private class ForumQuery extends MappingSqlQuery<Forum> {
        private ForumQuery(DataSource ds) {
            /* ⑤定义查询语句并预编译 */
    ...
    private class ForumInsert extends SqlUpdate {
        private ForumInsert(DataSource ds) {
            /* ③定义SQL语句并预编译 */
    ...
    private class GetTopicNum extends StoredProcedure {
        ...
        private GetTopicNum(DataSource ds) {
            setDataSource(ds);
            setSql(SQL);
            /* ③声明入参 */
        ...
        private int getTopicNum(int userId) {
            /* ⑤执行存储过程并返回结果 */
